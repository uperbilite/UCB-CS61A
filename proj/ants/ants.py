...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        def in_range(_n):
            return _n >= self.min_range and _n < self.max_range 

        nearest_place = self.place
        distance = 0

        while distance < self.min_range and nearest_place.is_hive is False:
            nearest_place = nearest_place.entrance
            distance += 1

        while in_range(distance) and len(nearest_place.bees) == 0 and nearest_place.is_hive is False:
            nearest_place = nearest_place.entrance
            distance += 1
        if nearest_place.is_hive is True:
            return None
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        """
        # BEGIN Problem 12
        def double_damage(_self):
            if _self.doubled_damage is False:
                _self.damage *= 2
                _self.doubled_damage = True

        doubled_place = self.place.exit
        while doubled_place is not None: # haven't reached the end of tunnel
            if doubled_place.ant is not None:
                double_damage(doubled_place.ant) 
                if doubled_place.ant.is_container is True and doubled_place.ant.ant_contained is not None:
                    double_damage(doubled_place.ant.ant_contained)
            doubled_place = doubled_place.exit
...
class Bee(Insect):
    ...
    def __init__(self, health):
        ...
        self.scared_turns = 0
        self.has_scared = False # Check if one bee has been scared once
        super().__init__(health)
    ...
    def blocked(self):
        ...
        if self.place.ant is None or (self.place.ant is not None and self.place.ant.blocks_path is False):
            return False
        return True
    ...
    def action(self, gamestate):
        ...
        def check_status(_self):
            if _self.slowed_turns <= 0:
                _self.slowed_turns = 0
                _self.is_slowed = False
            if _self.scared_turns <= 0:
                _self.scared_turns = 0
                _self.is_scared = False

        # If a bee is scared, change its moving destination
        if self.is_scared is True:
            if self.place.entrance.is_hive is False:
                destination = self.place.entrance
            else:
                destination = self.place

        if self.is_slowed is True:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        if self.place.ant.is_container is True:
            insects_and_distances[self.place.ant] = distance
        for bee in self.place.bees:
            insects_and_distances[bee] = distance

        front_place = self.place.entrance
        distance += 1

        while front_place.is_hive is False:
            if front_place.ant is not None:
                insects_and_distances[front_place.ant] = distance
            for bee in front_place.bees:
                insects_and_distances[bee] = distance
            front_place = front_place.entrance
            distance += 1
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        actual_damage = self.damage - distance * 0.25 - self.insects_shot * 0.0625
        if actual_damage <= 0:
            actual_damage = 0
