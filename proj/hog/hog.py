...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        running_high_next = running_high
        if diff > running_high:
            print(diff, 'point(s)! That\'s a record gain for Player', str(who) + '!')
            running_high_next = diff
        last_score_next = score
        return announce_highest(who, last_score_next, running_high_next)
    return say
    # END PROBLEM 7
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    num = picky_piggy_strategy(score, opponent_score, cutoff, num_rolls)
    if hog_pile(score + picky_piggy(opponent_score), opponent_score) != 0:
        return 0
    else:
        return num
    # END PROBLEM 11
