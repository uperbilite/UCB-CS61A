...
def about(topic):
    ...
    def is_contain(paragraph):
        word_list = split(remove_punctuation(lower(paragraph)))
        for x in word_list:
            for y in topic:
                if x == y:
                    return True
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    ans = typed_word
    lowest = limit
    # reveres to return the first string when multiple strings have the same lowest difference
    for x in reversed(valid_words): 
        if typed_word == x:
            return x
        if diff_function(typed_word, x, limit) <= lowest:
            ans = x
            lowest = diff_function(typed_word, x, limit)
    return ans
    # END PROBLEM 5
...
def time_per_word(words, times_per_player):
    ...
    for x in times_per_player:
        times += [[x[i + 1] - x[i] for i in range(len(x)) if i + 1 < len(x)]]
    ...
    for i in player_indices:
        ...
        for j in word_indices:
            ...
            for k in player_indices:
                if get_times(match)[k][j] < get_times(match)[min_index][j]:
                    min_index = k
            if min_index == i:
                player_fastest_words += [j]
        ans += [[get_words(match)[x] for x in player_fastest_words]]
    return ans
    # END PROBLEM 10
