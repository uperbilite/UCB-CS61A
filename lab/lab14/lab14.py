...
def add_trees(t1, t2):
    ...
    for branch1, branch2 in zipped_list:
        new_branches += [add_trees(branch1, branch2)]
    length = len(zipped_list)
    if length < len(t1.branches):
        for left in t1.branches[length:]:
            new_branches += [left]
    if length < len(t2.branches):
        for left in t2.branches[length:]:
            new_branches += [left]
