...
def cs_classes(post):
    ...
    True
    """
    return bool(re.search(r'\b[Cc][Ss]\s?\d+[ABCabc]\b', post))
...
def match_time(text):
    ...
    ['2:00']
    """
    return re.findall(r'(?:2[0-3]|1\d|0?\d):(?:[1-5]\d|0\d)(?:AM|PM)?', text)
...
def most_common_code(text):
    ...
    for code in code_list:
        if code_list.count(code) > max_count:
            max_count = code_list.count(code)
            result = code
    return result
