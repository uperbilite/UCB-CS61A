...
class Coin:
    ...
    def worth(self):
        extra_worth = Mint.present_year - self.year - 50
        if extra_worth <= 0:
            extra_worth = 0
...
def store_digits(n):
    ...
    if n < 10: # one digit
        ...
        while copy != 0:
            ...
            count += 1
        div = pow(10, count - 1)
        return Link(n // div, store_digits(n % div))
...
def two_list(vals, amounts):
    ...
    """
    "*** YOUR CODE HERE ***"
    if len(vals) == 1 and amounts[0] == 1:
        return Link(vals[0])
    elif amounts[0] == 1:
        label = vals[0]
        vals = vals[1:]
        amounts = amounts[1:]
        return Link(label, two_list(vals, amounts))
    else:
        amounts[0] -= 1
...
class VirFib():
    ...
    def next(self):
        if self.value == 0:
            ...
            result = VirFib(self.pre_value + self.value)
        result.pre_value = self.value
        return result
...
def is_bst(t):
    ...
    def bst_min(r):
        ...
        for branch in r.branches:
            min_branch = bst_min(branch)
            if min_branch < result:
                result = min_branch
        if r.label < result:
            result = r.label
        return result
    ...
    def bst_max(r):
        ...
        result = float('-inf')
        for branch in r.branches:
            max_branch = bst_max(branch)
            if max_branch > result:
                result = max_branch
        if r.label > result:
            result = r.label
        return result
